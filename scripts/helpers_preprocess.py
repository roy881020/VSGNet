...
def get_detections(segment_key, flag):
    if flag == 'train':
        annotation = ANNOTATIONS_train[str(segment_key)] # person bbox, action, object bbx not class
    ...
    #VCOCO ANNOTATION has no object class infomation
    annotation = clean_up_annotation(annotation) #annotation = [human, hois] from vcoco_label
    #pdb.set_trace()
    with open(cur_obj_path_s) as fp:
        detections = json.load(fp) #detections = [object, bbx] from coco label
    ...
    #pdb.set_trace()
    d_p_boxes, scores_persons, class_id_humans = get_boxes_det(persons_d, img_H, img_W) # from COCO
    #pdb.set_trace()
    d_o_boxes, scores_objects, class_id_objects = get_boxes_det(objects_d, img_H, img_W) # from COCO
    ...
    if len(d_o_boxes) > select_threshold - 1:
        d_o_boxes, scores_objects, class_id_objects = d_o_boxes[0:select_threshold - 1], scores_objects[
...
def get_compact_label(segment_key, flag): #segment_key : image_id(e.g. 319492), flag  : train, val, test
    d_p_boxes, d_o_boxes, scores_persons, scores_objects, class_id_humans, class_id_objects, annotation, shape = get_detections(
        segment_key, flag) #d_p_boxes -> detected person boxes(GT)
    #annotation -> from V-COCO
    # import pdb; pdb.set_trace()
