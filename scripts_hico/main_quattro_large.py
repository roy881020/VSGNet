...
from train_test_quattro_large import train_test
import model_large as rr
import random
...
parser.add_argument('-e', '--number_of_epochs', type=int, required=False, default=10,
...
parser.add_argument('-b', '--breaking_point', type=int, required=False, default=0,
                    help='Number of iteration to break the training loop')
parser.add_argument('-sa', '--saving_epoch', type=int, required=False, default=1000,
                    help='In which epoch to save e.g. 10 means the model would be saved every tenth epoch while training')
...
parser.add_argument('-v_i', '--Visualize', type=str, required=False, default='f',
                    help='Mention which phase(train,val,test) you want to visualize')
parser.add_argument('-c', '--Check_point', type=str, required=False, default='best',
...
hico_train = hico_Dataset(annotation_train, image_dir_train,
                          transform=transforms.Compose([Rescale((800, 800)), ToTensor()]))
