...
class hico_Dataset(Dataset):
    ...
    def __init__(self, json_file_image, root_dir, transform=None):
        ...
        if self.flag == 'train':
            self.hico_frame = [x for x in self.hico_frame_file.keys() if x not in str(bad_detections_train)]
        elif self.flag == 'test':
            self.hico_frame = [x for x in self.hico_frame_file.keys() if x not in str(bad_detections_test)]
