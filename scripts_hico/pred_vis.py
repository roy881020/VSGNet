...
def visual(image_id, flag, pairs_info, score_HOI, score_interact, score_obj_box, score_per_box, score_REL,
           score_HOI_pair, ground_truth):
    start = 0
    for batch in range(len(image_id)):
        ...
        if flag == 'train':
            ...
            image_dir_s = image_dir_train + "/HICO_train2015_%.8i.json" % (this_image)

        elif flag == 'test':

            cur_obj_path_s = OBJ_PATH_test_s + "HICO_train2015_%.8i.json" % (this_image)
            image_dir_s = image_dir_test + "/HICO_train2015_%.8i.json" % (this_image)
        ...
        for person_box in person_bbx:
            for object_box in obj_bbx:
                ...
                pred.append(('TOTAL_PREDICTION', [(ID2VERB[ind], float("%.2f" % score_HOI_this_sample[ind])) for ind in
                                                  np.argsort(score_HOI_this_sample)[-5:][::-1]]))
                prediction = pd.DataFrame(pred, columns=['Name', 'Prediction'])

                img = cv2.imread(image_dir_s, 3)
                x, y, w, h = int(person_box[0]), int(person_box[1]), int(person_box[2] - person_box[0]), int(
                    person_box[3] - person_box[1])
                cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 3)
                x, y, w, h = int(object_box[0]), int(object_box[1]), int(object_box[2] - object_box[0]), int(
